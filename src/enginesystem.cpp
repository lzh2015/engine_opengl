...
void EngineSystem::Run()
{
	...
	while(!done)
	{
		...
		//there is a key input lag issue if using following else
//		else
//		{
//			// Otherwise do the frame processing.
//			result = Frame();
...
LRESULT CALLBACK EngineSystem::MessageHandler(HWND hwnd, UINT umsg, WPARAM wparam, LPARAM lparam)
{
	switch(umsg)
	{
		...
		case WM_KEYDOWN:
		{
			//cout << (unsigned int)wparam <<endl;
			// If a key is pressed send it to the input object so it can record that state.
			if ((lparam & (1<<30)) == 0)
			{
				Input::GetInstance().KeyDown((unsigned int)wparam);
				m_Graphics->OnKeyboard((unsigned int)wparam);
			}
		...
		case WM_LBUTTONDOWN:
		{
			...
			int x = LOWORD(lparam);
			int y = HIWORD(lparam);
			Input::GetInstance().MouseClick(MOUSE_LEFT_BUTTON, state, x, y);
			return 0;
		}
...
bool EngineSystem::InitializeWindows(int& screenWidth, int& screenHeight)
{
	WNDCLASSEX wc;
	...
	m_hwnd = CreateWindowExW(WS_EX_APPWINDOW, m_applicationName, m_applicationName, WS_POPUP,
...
//		screenHeight = 720;
	...
	result = m_OpenGL->InitializeOpenGL(m_hwnd, screenWidth, screenHeight);
	if(!result)
	{
